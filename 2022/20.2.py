...
def print_values(node, n, key = 1):
    # node = starting node, n = number of nodes; key = mutliplying factor (for part 2)
    # prints values of nodes
    A = [node.val * key]
    for _ in range(n-1):
        node = node.next
        A.append(node.val * key)
    print(A)
...
for _ in range(10):
    for node in nodes:
        if node.val > 0:
            for _ in range( (node.val * key) % (len(nodes)-1) ):
                move_node(node)
        elif node.val < 0:
            for _ in range( (-node.val * key) % (len(nodes)-1) ):
                move_node_left(node)
