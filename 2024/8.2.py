...
for x in frequencies:
    ...
    for i in range(len(locations)):
        ...
        for j in range(i+1, len(locations)):
            ...
            # add while within map
            x1_new, y1_new = x1,y1
            x2_new, y2_new = x2,y2
            while x1_new >= 0 and x1_new < n and y1_new >= 0 and y1_new < m:
                anti_node_locations.add((x1_new, y1_new))

                x1_new -= (x2-x1)
                y1_new -= (y2-y1)

            while x2_new >= 0 and x2_new < n and y2_new >= 0 and y2_new < m:
...
print(len(anti_node_locations)) # part 2: 1280
